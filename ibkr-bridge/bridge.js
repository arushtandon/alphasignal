...
const { asiaUnfilledRearmReason, LSE_THROUGH_SIT_MS } = require('../lib/ibkr/asia-entry-rearm');
...
function parentEntrySpec(contract, action, qty, opts = {}) {
  ...
  if (phase === 'rth') {
    ...
    // stuck at 3482.5 on 8 Sep) leaves the buy unfilled all morning. Through-
    // limit stays close to last (0.5%) — walking 2% tripped IB 2161.
    if (contract.market === 'LSE') {
      const ref = quotePx > 0 ? quotePx : entryPx;
      if (ref > 0) {
        const sell = side === 'sell';
        const thru = Number(opts.throughPct) > 0 ? Number(opts.throughPct) : LSE_THROUGH_PCT;
        const raw = sell ? ref * (1 - thru) : ref * (1 + thru);
        return {
          orderType: 'LMT', action, totalQuantity: qty,
          lmtPrice: roundPx(raw, contract, sell ? 'down' : 'up'),
          tif: 'DAY', outsideRth: ORDER_OUTSIDE_RTH, transmit: true, entryStyle: 'LMT-THROUGH'
...
async function main() {
  ...
  function pickExecSlot() {
    return pickLeastBusy(execSlots, execRr++, { workersOnly: true })
      || slotByClientId(activeClientId) || execSlots[0] || null;
  }
  ...
  if (!DRY) {
    ...
    ib.on(EventName.orderStatus, (orderId, status, filled, remaining, avgFillPrice, permId, parentId, lastFillPrice) => {
      ...
      const watched = rowOwningOrder(state.byKey, orderId);
      if (watched || st === 'Inactive' || st === 'Filled' || st === 'Cancelled' || st === 'ApiCancelled') {
        log('orderStatus', (watched && watched.ticker) || '', 'oid=' + orderId, st,
          'filled=' + filled, 'left=' + remaining, 'avg=' + avgFillPrice);
      }
        ...
        for (const [key, row] of Object.entries(state.byKey)) {
          ...
          if (role === 'entry') {
            ...
            if (row.rollInId != null && Number(row.rollInId) === orderId) {
              row.parentId = orderId;
              row.rollInId = null;
              row.rollInFlight = false;
              log('FUTURES ROLL in-fill', key, (row.contract && row.contract.localSymbol) || row.ticker, px);
            }
            ...
            if (firstEntry) notifyFillTelegram('entry', key, row, row.ibAvgFill || px, exec.shares);
          }
          if (role === 'stop') {
            notifyFillTelegram('stop', key, row, px, exec.shares);
          }
          ...
          saveState(state);
          if (role === 'flatten' && row.futuresRollPending && !row.rollInId) {
            void completeFuturesRollIn(key, row);
          }
  ...
  async function resolveInstrument(contract) {
    ...
    if (cached && !contract.rollExcludeConId && (Date.now() - cached.at) < 6 * 3600 * 1000 && cached.contract) {
      Object.assign(contract, cached.contract);
      contract.needsFrontMonth = false;
      return contract;
    }
    const specs = orderedCommoditySpecs(yKey, contract.entryPx);
    if (specs.length && specFields(specs[0]) && !contract.rollKeepSpec) {
    ...
    const reqOneFutFront = (spec) => new Promise(resolvePick => {
      ...
      const finish = () => {
        ...
        const excludeId = Number(contract.rollExcludeConId) || 0;
        const excludeMonth = String(contract.rollExcludeMonth || '').replace(/\D/g, '').slice(0, 8);
        const live = pool
          .filter(x => futuresStillTradable(x.month, today))
          .filter(x => !(excludeId > 0 && Number(x.conId) === excludeId))
          .filter(x => {
            if (!excludeMonth) return true;
            return String(x.month || '').replace(/\D/g, '').slice(0, 8) !== excludeMonth;
          })
    ...
    const trySpecs = (contract.rollKeepSpec && contract.symbol)
      ? [{
  ...
  async function flushMarks() {
    ...
    for (const row of Object.values(state.byKey || {})) {
      ...
      if (!mark) continue;
      mark.localSymbol = row.contract.localSymbol || null;
      mark.lastTradeDateOrContractMonth = row.contract.lastTradeDateOrContractMonth || null;
      mark.futLabel = futuresLocalMonthLabel(row.contract.localSymbol)
        || String(row.contract.lastTradeDateOrContractMonth || '').slice(0, 8);
      mark.futExpired = futuresExpired(row.contract);
    }
  ...
  function transmitOrder(orderId, contract, order, label, exchangeOverride) {
    ...
    const oc = placeableContract(contract, exchangeOverride);
  ...
  async function placeBracket(evt) {
    ...
    if (DRY || !ib) {
      log('DRY bracket', evt.ticker, evt.side, JSON.stringify({ contract, parent, stopOrder, tp1Order, split, entryStyle, phase: sessionPhase(contract), quotePx, quoteSrc, asiaStandalone, placeEx }, null, 1));
    } else {
      const oc = placeableContract(contract, placeEx);
  ...
  function onOrderStatus(orderId, status, filled, avgFillPrice) {
    ...
    for (const [key, row] of Object.entries(state.byKey)) {
      ...
      if (row.parentId === orderId && filled > 0 && !row.entryFilled) {
        ...
        applyFillRebase(key, row, row.ibAvgFill);
        scheduleProtectiveBracket(key);
        notifyFillTelegram('entry', key, row, row.ibAvgFill || avgFillPrice, filled);
        saveState(state);
      ...
      if (row.stopId === orderId && status === 'Filled') {
        ...
        row.closed = true;
        notifyFillTelegram('stop', key, row, avgFillPrice, filled);
  ...
  function collectRiskFindings(keyState, reconResp) {
    ...
    for (const [key, row] of Object.entries(state.byKey || {})) {
      ...
      if (futuresDueForRoll(row.contract)) {
        logOnce('fut-exp-prot-' + key, 'skip protective attach — rolling expired futures',
          row.contract.localSymbol || row.ticker, row.contract.lastTradeDateOrContractMonth);
        continue;
      }
      const phase = sessionPhase(row.contract);
  ...
  function notifyFillTelegram(role, key, row, px, qty) {
    if (DRY || !telegramConfigured() || !row) return;
    const flag = role === 'entry' ? 'entryTelegramSent'
      : role === 'stop' ? 'stopTelegramSent' : null;
    if (flag && row[flag]) return;
    if (flag) row[flag] = true;
    const side = row.side === 'sell' ? 'SHORT' : 'LONG';
    const title = role === 'entry' ? 'Entry filled'
      : role === 'stop' ? 'Stop filled' : 'Fill';
    const emoji = role === 'stop' ? '🔴' : '🟢';
    const qtyBit = qty != null && qty !== '' ? String(qty) : String(row.qtyTotal || '');
    const pxBit = px != null && Number(px) > 0 ? String(px) : String(row.ibAvgFill || '');
    const exch = String(row.placeExchange || (row.contract && row.contract.exchange) || '').toUpperCase();
    const msg = emoji + ' <b>' + title + '</b>\n'
      + String(row.ticker || key) + ' · ' + (row.hz || 'short') + ' ' + side + '\n'
      + qtyBit + (pxBit ? (' @ ' + pxBit) : '')
      + (exch ? (' · ' + exch) : '');
    sendTelegramAlert(msg, { html: true })
      .then(() => log('TELEGRAM:', title, 'sent', key))
      .catch(e => {
        if (flag) row[flag] = false;
        log('TELEGRAM:', title, 'failed', e.message);
      });
  }
  ...
  function heldForConId(conId) {
    const cid = Number(conId);
    if (!(cid > 0)) return null;
    for (const v of posMap.values()) {
      if (v && v.contract && Number(v.contract.conId) === cid) return v;
    }
  ...
  function queueRollSettleReport(key, row, settlePx) {
    if (row.rollSettleReported) return;
    const qty = Math.max(1, Number(row.qtyRunner) > 0 && row.tp1Done
      ? Number(row.qtyRunner) : Number(row.qtyTotal) || 1);
    const fillAt = new Date().toISOString();
    const cMeta = enrichSessionMeta(row.contract);
    const phase = sessionPhase(cMeta || {}, Date.parse(fillAt));
    state.pendingReports = state.pendingReports || [];
    state.pendingReports.push({
      kind: 'exec',
      execId: 'roll-settle-' + key + '-' + (row.contract && row.contract.conId || 'x'),
      key,
      ticker: row.ticker,
      hz: row.hz || 'short',
      ...
      orderId: null,
      qty,
      price: +Number(settlePx).toFixed(6),
      currency: (row.contract && row.contract.currency) || 'USD',
      ccyScale: 1,
      multiplier: Number(row.contract && row.contract.multiplier) || null,
      ibSymbol: row.contract && row.contract.symbol || null,
      session: phase,
      sessionLabel: sessionLabel(phase),
      recon: 'futures-roll',
    ...
    row.rollSettlePx = Number(settlePx);
  }
  ...
  async function completeFuturesRollIn(key, row) {
    ...
    const stub = {
      ...
      tick: old.tick,
      tradingClass: old.tradingClass,
      market: old.market || 'GLOBE',
      lotHint: 1,
      needsFrontMonth: true,
      usRth: false,
    ...
    if (!next || !(next.conId > 0) || Number(next.conId) === Number(old.conId)) {
      log('FUTURES ROLL in failed — no next month', key, next && next.localSymbol);
      row.rollInFlight = false;
      saveState(state);
      return;
    }
    row.rolledFrom = {
      localSymbol: old.localSymbol,
      conId: old.conId,
      lastTradeDateOrContractMonth: old.lastTradeDateOrContractMonth,
      settlePx
    };
    row.contract = next;
    row.stopId = null;
    row.tp1Id = null;
    const qty = Math.max(1, Number(row.qtyTotal) || 1);
    const oid = nidForRow(row);
    row.rollInId = oid;
    row.parentId = oid;
    row.futuresRollPending = false;
    row.rollInFlight = true;
    row.orderSubmittedAt = new Date().toISOString();
    saveState(state);
    transmitOrder(oid, next, baseOrder({
      orderId: oid,
      ...
      outsideRth: true,
      transmit: true
    }), 'futures roll-in ' + key + ' ' + (next.localSymbol || next.symbol));
    log('FUTURES ROLL in', key, old.localSymbol, '→', next.localSymbol,
      'settle=' + settlePx, 'qty=' + qty);
  }

  async function maybeRollExpiredFutures(key, row) {
    ...
    transmitOrder(fid, row.contract, baseOrder({
      ...
      action: row.side === 'sell' ? 'BUY' : 'SELL',
      orderType: 'MKT',
      totalQuantity: posInDir,
      tif: 'DAY',
      outsideRth: true,
      transmit: true
    }), 'futures roll-out ' + key);
    log('FUTURES ROLL out', key, row.contract.localSymbol, 'qty=' + posInDir, 'settle~' + settlePx);
  }

  async function rollExpiredFutures() {
    for (const [key, row] of Object.entries(state.byKey || {})) {
      try {
        await maybeRollExpiredFutures(key, row);
      } catch (e) {
        log('FUTURES ROLL failed', key, e.message);
      }
    }
  }
  ...
  async function reconcilePositions() {
    ...
    try {
      ...
      //     LMT-THROUGH (LSE native MKT sits at last). Walk the through-limit
      //     like JP when last has crossed it or it sits marketable 10 min.
        ...
        try {
          ...
          if (placed) {
            ...
            log('RECONCILE: re-armed', key, 'reason=' + reason, '→', placed.entryStyle,
              ...
              placed.placeExchange ? ('exch=' + placed.placeExchange) : '');
          } else {
            row.lastRearmAt = new Date().toISOString();
      ...
      for (const [key, row] of Object.entries(state.byKey)) {
        ...
        if (falseOrphanFlat && (row.entryFilled || parentFilledQty > 0) && phase === 'rth') {
          ...
          if (!row.entryFilled && parentFilledQty > 0) {
            row.entryFilled = true;
            notifyFillTelegram('entry', key, row, row.ibAvgFill, parentFilledQty);
            saveState(state);
          }
          continue;
        }
        if (posInDir > 0) {
          if (!row.entryFilled) {
            row.entryFilled = true;
            notifyFillTelegram('entry', key, row, row.ibAvgFill, posInDir);
            saveState(state);
        ...
        if (asia) {
        ...
        } else if (eu && phase === 'rth' && row.entryStyle === 'LMT-THROUGH' && !row.entryFilled) {
          // Same walk as JP: delayed LSE last left SHEL.L sitting at 3586 on 8 Sep.
          ensureMktData(row.ticker, contract);
          const q = await fetchEntryQuote(row.ticker, 'rth', row.side);
          const quotePx = Number(q && q.px) || Number(ibQuoteForTicker(row.ticker)) || 0;
          reason = asiaUnfilledRearmReason({
            phase,
            entryStyle: row.entryStyle,
            stopId: row.stopId,
            ...
            orderSubmittedAt: row.orderSubmittedAt,
            now: Date.now(),
            minutesSinceRth: minutesSinceEuRth(),
            auctionHoldMin: AUCTION_HOLD_MIN,
            side: row.side,
            quotePx,
            extLmt: Number(row.extLmt) || 0,
          ...
          if (reason === 'asia-rth-reprice') {
            log('RECONCILE: LSE/EU through-limit stale', key,
              'quote=', quotePx, 'lmt=', row.extLmt, 'side=', row.side,
              'exch=', row.placeExchange || '');
          } else if (reason === 'asia-rth-retry') {
            log('RECONCILE: LSE/EU through-limit still unfilled — rotate venue', key,
              'quote=', quotePx, 'lmt=', row.extLmt, 'side=', row.side,
              'from=', row.placeExchange || preferredExchange(contract));
          }
      ...
      for (const [key, row] of Object.entries(state.byKey)) {
        ...
        if (row.futuresRollPending || row.rollInFlight || row.rollInId) continue;
        const held = heldForContract(row.contract);
    ...
    if (!forceReconcile && positionsReady) {
      for (const row of Object.values(state.byKey)) {
        ...
        if ((euMkt === 'XETRA' || euMkt === 'EURONEXT' || euMkt === 'LSE')
          && sessionPhase(row.contract) === 'rth'
          && (row.entryStyle === 'OPG' || row.entryStyle === 'MKT'
            || row.entryStyle === 'LMT-THROUGH' || row.entryStyle === 'LMT-OPEN'
            || row.contractRejected)) {
          forceReconcile = true;
          break;
        }
      }
