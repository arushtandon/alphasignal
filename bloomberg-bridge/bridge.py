...
def _bloomberg_soft(exc: BaseException) -> bool:
    ...
    return any(
        ...
        for t in (
            ...
            "BAD_FIELD",
            "SECURITY NOT FOUND",
            # pdblp / blpapi: status events landed on synchronous ref(); common after reconnect.
            "SERVICE_STATUS",
    ...
    for attempt in range(3):
        try:
            ...
            if _session_dead(exc):
                raise
            em = str(exc).upper()
            transient = attempt < 2 and (
                "SERVICE_STATUS" in em or "UNEXPECTED EVENT TYPE" in em or "REFERENCE ERROR" in em
            )
            # Per-field soft failures are common; never abort the whole /snapshot for one field.
            if not _bloomberg_soft(exc) and not transient:
                try:
                    import sys
