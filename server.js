...
async function fmpEarningCalendarByRange(fromISO, toISO) {
  ...
  const url = `https://financialmodelingprep.com/api/v3/earning_calendar?from=${encodeURIComponent(fromISO)}&to=${encodeURIComponent(toISO)}&apikey=${encodeURIComponent(k)}`;
  try {
    const r = await fetch(url, { signal: AbortSignal.timeout(22000) });
    if (!r.ok) {
      console.warn('FMP calendar', r.status);
      return [];
    }
    ...
    console.warn('FMP calendar', e.message);
    return [];
  }
}
...
async function yahooEarningsGapRow(ticker) {
  try {
    const qs = await quoteSummary(ticker, 'calendarEvents,summaryProfile');
...
async function mergedEarningsCalendarWidget(fromISO, toISO) {
  ...
  for (const [, row] of byTicker) {
    if (
      ...
      const hit = fmpRows.find((r) => normalizeTickerMatch(r.symbol) === normalizeTickerMatch(row.ticker));
      const nm =
...
app.get('/api/earnings/:symbol', async (req, res) => {
  ...
  try {
    ...
    const fhVariants = sym.includes('.') ? [sym, sym.replace(/\./g, '-')] : [sym];
    let fhRows = [];
    ...
    if (!nextDate && process.env.FMP_API_KEY) {
      ...
      const hit = fmpArr.find((r) => normalizeTickerMatch(r.symbol) === normalizeTickerMatch(sym));
      if (hit?.date) {
    ...
    const fromCal = nextEarningsFromCalendar(qs);
    ...
    for (const host of ['query1', 'query2']) {
      try {
        ...
        if (!result) continue;
        epsHistory = earningsHistoryFromChart(result);
        if (!nextDate) {
          const nowTs = Date.now() / 1000;
          const evts = Object.values(result.events?.earnings || {}).sort((a, b) => a.date - b.date);
...
function tryParseJsonArray(str) {
  ...
  if (fi !== -1 && li > fi) {
    try {
      const r = JSON.parse(str.slice(fi, li + 1).replace(/,\s*([}\]])/g, '$1'));
      if (Array.isArray(r) && r.length) return r;
    } catch (_) {}
  }
  const oi = str.indexOf('{');
  const oe = str.lastIndexOf('}');
  if (oi !== -1 && oe > oi) {
    try {
      const one = JSON.parse(str.slice(oi, oe + 1).replace(/,\s*([}\]])/g, '$1'));
...
/** Mid-range % moves per horizon; buy = long bias, sell = short bias */
const HORIZON_PCT = {
  short: {
    buy: { tp1: 0.03, tp2: 0.055, sl: -0.015 },
    sell: { tp1: -0.03, tp2: -0.055, sl: 0.015 }
  },
  medium: {
    buy: { tp1: 0.09, tp2: 0.16, sl: -0.05 },
    sell: { tp1: -0.09, tp2: -0.16, sl: 0.05 }
  },
  long: {
    buy: { tp1: 0.22, tp2: 0.45, sl: -0.10 },
    sell: { tp1: -0.22, tp2: -0.45, sl: 0.10 }
  }
};
...
app.get('/api/earnings-calendar', async (req, res) => {
  ...
  if (!req.query.force && calCache && Date.now() - calTs < 21600000) {
