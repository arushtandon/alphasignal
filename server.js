...
app.get('/api/earnings/:symbol', async (req, res) => {
  ...
  try {
    // Yahoo Finance earnings history and upcoming
    const urls = [
      `https://query1.finance.yahoo.com/v10/finance/quoteSummary/${encodeURIComponent(sym)}?modules=earnings,earningsTrend,calendarEvents`,
      `https://query2.finance.yahoo.com/v10/finance/quoteSummary/${encodeURIComponent(sym)}?modules=earnings,earningsTrend,calendarEvents`,
    ];
    let data = null;
    for(const url of urls) {
      try {
    ...
    if(!data || !data.quoteSummary || data.quoteSummary.error) {
      return res.status(404).json({ error: 'No earnings data' });
    }
    const result = data.quoteSummary.result?.[0] || {};

    // Extract upcoming earnings date
    const cal = result.calendarEvents;
    const earningsDate = cal?.earnings?.earningsDate?.[0]?.raw
      ? new Date(cal.earnings.earningsDate[0].raw * 1000).toISOString().slice(0, 10)
      : null;
    const earningsDateEnd = cal?.earnings?.earningsDate?.[1]?.raw
      ? new Date(cal.earnings.earningsDate[1].raw * 1000).toISOString().slice(0, 10)
      : null;
    const epsEst = cal?.earnings?.epsAverage?.fmt || null;
    const earningsTime = cal?.earnings?.earningsCallTime?.fmt || null;

    // Extract historical quarterly earnings
    const history = [];
    const earningsHist = result.earnings?.financialsChart?.quarterly || [];
    const earningsActual = result.earnings?.earningsChart?.quarterly || [];

    earningsActual.forEach(q => {
      history.push({
        quarter: q.date,
        epsActual: q.actual?.fmt || null,
        epsEstimate: q.estimate?.fmt || null,
        epsSurprise: null, // calculated below
        revenueActual: null, // from financials
        stockReaction: null  // requires price data
      });
    });

    // Get surprise %
    const surpriseData = result.earnings?.earningsChart?.quarterly || [];
    surpriseData.forEach((q, i) => {
      if(history[i] && q.actual?.raw != null && q.estimate?.raw != null) {
        const surprise = ((q.actual.raw - q.estimate.raw) / Math.abs(q.estimate.raw) * 100);
        history[i].epsSurprise = (surprise >= 0 ? '+' : '') + surprise.toFixed(1) + '%';
        history[i].beat = surprise >= 0;
      }
    });

    // Revenue from financials chart
    const revData = result.earnings?.financialsChart?.quarterly || [];
    revData.forEach((q, i) => {
      if(history[i]) {
        history[i].revenueActual = q.revenue?.fmt || null;
        history[i].revenueEstimate = null; // Yahoo doesn't provide quarterly rev estimates easily
      }
    });
    ...
    res.json({
      ...
      nextEarningsDateEnd: earningsDateEnd,
      epsEstimate: epsEst,
      earningsTime: earningsTime,
      history: history.slice(0, 4)
    });
  } catch(e) {
    console.error('Earnings API error:', e.message);
