...
app.get('/api/earnings/:symbol', async (req, res) => {
  ...
  try {
    let nextDate=null, epsHistory=[], epsEst=null, callTime=null;
    // Step 1: Yahoo v8 chart with earnings events (no crumb required)
    for(const host of ['query1','query2']) {
      try {
        const url = `https://${host}.finance.yahoo.com/v8/finance/chart/${encodeURIComponent(sym)}?range=3y&interval=3mo&events=earnings%7Cdividends&includePrePost=false`;
        const r = await fetch(url, {headers:YF_HEADERS, signal:AbortSignal.timeout(8000)});
        if(!r.ok) continue;
        ...
        const future = evts.filter(e=>e.date>nowTs);
        const past = evts.filter(e=>e.date<=nowTs);
        if(future[0]) nextDate = new Date(future[0].date*1000).toISOString().slice(0,10);
        epsHistory = past.slice(-4).reverse().map(e=>{
          ...
          return {quarter:new Date(e.date*1000).toLocaleDateString('en-GB',{month:'short',year:'numeric'}),
            ...
            beat:surp!=null?surp>=0:null, revenueActual:null};
        });
        break;
      } catch(e){console.log('Chart err:',e.message);}
    }
    // Step 2: Claude web search for next date if not found
    const apiKey = process.env.ANTHROPIC_API_KEY;
    if(!nextDate && apiKey) {
      try {
        const cr = await fetch('https://api.anthropic.com/v1/messages',{
          method:'POST', headers:{'Content-Type':'application/json','x-api-key':apiKey,'anthropic-version':'2023-06-01','anthropic-beta':'tools-2024-04-04'},
          body:JSON.stringify({model:'claude-haiku-4-5-20251001',max_tokens:400,
            tools:[{type:'web_search_20250305',name:'web_search'}],
            system:'Financial assistant. Today='+todayISO+'. Return ONLY JSON, no markdown.',
            messages:[{role:'user',content:'Search: next earnings date for '+sym+' stock after '+todayISO+'. Return ONLY JSON: {"nextDate":"YYYY-MM-DD","earningsTime":"pre-market|post-market","epsEstimate":"number","quarter":"Q1 FY2026"}'}]}),
          signal:AbortSignal.timeout(15000)});
        if(cr.ok){
          const cd=await cr.json();
          let raw=''; if(cd.content)cd.content.forEach(b=>{if(b.type==='text')raw+=b.text;});
          raw=raw.replace(/```json/gi,'').replace(/```/g,'').trim();
          const si=raw.indexOf('{'),ei=raw.lastIndexOf('}');
          if(si!==-1&&ei>si){try{const p=JSON.parse(raw.slice(si,ei+1));if(p.nextDate&&p.nextDate>=todayISO){nextDate=p.nextDate;epsEst=p.epsEstimate||null;callTime=p.earningsTime||null;}}catch(pe){console.log('date parse:',pe.message);}}
        }
      } catch(e){console.log('Claude next date err:',e.message);}
    }
    // Step 3: Claude web search for historical EPS if not found
    if(!epsHistory.length && apiKey) {
      try {
        const cr = await fetch('https://api.anthropic.com/v1/messages',{
          method:'POST', headers:{'Content-Type':'application/json','x-api-key':apiKey,'anthropic-version':'2023-06-01','anthropic-beta':'tools-2024-04-04'},
          body:JSON.stringify({model:'claude-haiku-4-5-20251001',max_tokens:800,
            tools:[{type:'web_search_20250305',name:'web_search'}],
            system:'Financial data. Return ONLY valid JSON array, no markdown. Real data only.',
            messages:[{role:'user',content:'Search: '+sym+' earnings results last 4 quarters. Return ONLY JSON array: [{"quarter":"Q4 FY2025","date":"YYYY-MM-DD","epsActual":"1.86","epsEstimate":"1.79","epsSurprise":"+3.9%","revenueActual":"27.2B","stockReaction":"+2.8%","beat":true}]'}]}),
          signal:AbortSignal.timeout(15000)});
        if(cr.ok){
          const cd=await cr.json();
          let raw=''; if(cd.content)cd.content.forEach(b=>{if(b.type==='text')raw+=b.text;});
          raw=raw.replace(/```json/gi,'').replace(/```/g,'').trim();
          const si=raw.indexOf('['),ei=raw.lastIndexOf(']');
          if(si!==-1&&ei>si){try{epsHistory=JSON.parse(raw.slice(si,ei+1)).slice(0,4);}catch(pe){console.log('hist parse:',pe.message);}}
        }
      } catch(e){console.log('Claude history err:',e.message);}
    }
    res.json({symbol:sym,nextEarningsDate:nextDate,nextEarningsDateEnd:null,epsEstimate:epsEst,earningsTime:callTime,history:epsHistory.slice(0,4)});
  } catch(e){
