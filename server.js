...
async function finnhubEarningsCalendar(fromISO, toISO, opts = {}) {
  ...
  async function fetchOne(intlFlag) {
    ...
    try {
      ...
      if (!r.ok) {
        console.warn('Finnhub calendar', r.status, intlFlag || 'no-intl-flag');
        return [];
      }
      const j = await r.json();
  ...
  if (opts.symbol) {
    return fetchOne(opts.international === false ? 'false' : 'true');
  }
  ...
  merged.forEach((e) => {
    ...
    const k =
      normalizeTickerMatch(String(e.symbol)) + '_' + String(e.date || '').slice(0, 10);
...
function mapFinnhubCalRow(e) {
  ...
  return {
    ...
    date: normalizeVendorDate(e.date),
...
function calRowDateISO(e) {
  ...
  return d ? normalizeVendorDate(d) : '';
}
...
/** Finnhub/FMP raw row counts (7-day window); use to verify Render env vars and provider responses */
app.get('/api/earnings-probe', async (req, res) => {
  res.setHeader('Cache-Control', 'no-store');
  const todayISO = new Date().toISOString().slice(0, 10);
  const horizon = new Date();
  ...
  const endISO = horizon.toISOString().slice(0, 10);
  try {
    const fh = await finnhubEarningsCalendar(todayISO, endISO);
    const fmp = await fmpEarningCalendarByRange(todayISO, endISO);
    res.json({
      window: [todayISO, endISO],
      keys: { finnhub: !!finnhubToken(), fmp: !!fmpApiKey() },
      rows: { finnhub: fh.length, fmp: fmp.length },
      tip:
        finnhubToken() && fmpApiKey() && fh.length === 0 && fmp.length === 0
          ? 'Keys set but APIs returned zero rows — check plan limits / key spelling / Finnhub+FMP dashboards.'
          : null
    });
  } catch (e) {
    res.status(500).json({ error: e.message });
