...
app.get('/api/earnings/:symbol', async (req, res) => {
  ...
  try {
    ...
    let nextDate=null, nextDateEnd=null, epsEst=null, callTime=null, epsHistory=[];

    // Step 1: Try v10 quoteSummary (best data - works sometimes)
    const v10urls = [
      `https://query1.finance.yahoo.com/v10/finance/quoteSummary/${encodeURIComponent(sym)}?modules=earnings%2CcalendarEvents`,
      `https://query2.finance.yahoo.com/v10/finance/quoteSummary/${encodeURIComponent(sym)}?modules=earnings%2CcalendarEvents`,
    ];
    for(const url of v10urls) {
      try {
        const r = await fetch(url, { headers: YF_HEADERS, signal: AbortSignal.timeout(6000) });
        if(r.ok) {
          const d = await r.json();
          if(!d.quoteSummary?.error) {
            const qr = d.quoteSummary?.result?.[0];
            const cal = qr?.calendarEvents?.earnings;
            const dates = cal?.earningsDate || [];
            const d0 = dates[0]?.raw ? new Date(dates[0].raw*1000).toISOString().slice(0,10) : null;
            const d1 = dates[1]?.raw ? new Date(dates[1].raw*1000).toISOString().slice(0,10) : null;
            if(d0 && d0 >= todayISO) { nextDate=d0; nextDateEnd=d1; epsEst=cal?.epsAverage?.fmt||null; callTime=cal?.earningsCallTime?.fmt||null; }
            // EPS history
            const qHist = qr?.earnings?.earningsChart?.quarterly || [];
            const qRev  = qr?.earnings?.financialsChart?.quarterly || [];
            if(qHist.length) {
              epsHistory = qHist.map((q,i) => {
                const rv = qRev[i];
                const surp = q.actual?.raw!=null && q.estimate?.raw!=null
                  ? ((q.actual.raw - q.estimate.raw) / Math.abs(q.estimate.raw) * 100) : null;
                return { quarter:q.date, epsActual:q.actual?.fmt, epsEstimate:q.estimate?.fmt,
                  epsSurprise: surp!=null?(surp>=0?'+':'')+surp.toFixed(1)+'%':null,
                  revenueActual: rv?.revenue?.fmt||null, beat: surp!=null?surp>=0:null };
              });
            }
            break;
          }
    ...
    // Step 2: Try v8 chart with events for earnings dates (reliable server-side)
    if(!nextDate || !epsHistory.length) {
      const v8urls = [
        `https://query1.finance.yahoo.com/v8/finance/chart/${encodeURIComponent(sym)}?range=3y&interval=3mo&events=earnings&includePrePost=false`,
        `https://query2.finance.yahoo.com/v8/finance/chart/${encodeURIComponent(sym)}?range=3y&interval=3mo&events=earnings&includePrePost=false`,
      ];
        ...
        try {
          ...
          if(r.ok) {
            ...
            if(result) {
              ...
              const future = evts.filter(e=>e.date>now);
              const past   = evts.filter(e=>e.date<=now);
              if(!nextDate && future[0]) {
                nextDate = new Date(future[0].date*1000).toISOString().slice(0,10);
              }
              // If no EPS history, build from past events
              if(!epsHistory.length && past.length) {
                epsHistory = past.slice(-4).reverse().map(e => ({
                  quarter: new Date(e.date*1000).toLocaleDateString('en-GB',{month:'short',year:'numeric'}),
                  date: new Date(e.date*1000).toISOString().slice(0,10),
                  epsActual: e.epsActual ? String(e.epsActual) : null,
                  epsEstimate: e.epsEstimate ? String(e.epsEstimate) : null,
                  epsSurprise: (e.epsActual!=null && e.epsEstimate!=null)
                    ? ((((e.epsActual-e.epsEstimate)/Math.abs(e.epsEstimate))*100)>=0?'+':'')
    ...
    res.json({ symbol:sym, nextEarningsDate:nextDate, nextEarningsDateEnd:nextDateEnd,
      epsEstimate:epsEst, earningsTime:callTime, history:epsHistory.slice(0,4) });
  } catch(e) {
