...
async function fmpEarningCalendarByRange(fromISO, toISO) {
  ...
  try {
    ...
    if (!rows.length) {
      const stable = `https://financialmodelingprep.com/stable/earning_calendar?from=${encodeURIComponent(fromISO)}&to=${encodeURIComponent(toISO)}&apikey=${encodeURIComponent(k)}`;
      const r2 = await fetch(stable, { signal: AbortSignal.timeout(22000) }).catch(() => null);
      if (r2 && r2.ok) {
        try {
          const j2 = await r2.json();
          rows = Array.isArray(j2) ? j2 : [];
        } catch (_) {}
      }
    }
