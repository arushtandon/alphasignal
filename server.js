    ...
    if(!nextDate && apiKey) {
      try {
        ...
        if(cr.ok){
          ...
          raw=raw.trim().replace(/```[\s\S]*?```/g,'').trim();
          const si=raw.indexOf('{'),ei=raw.lastIndexOf('}');
          if(si!==-1){const p=JSON.parse(raw.slice(si,ei+1));if(p.nextDate>=todayISO){nextDate=p.nextDate;epsEst=p.epsEstimate||null;callTime=p.earningsTime||null;}}
        }
    ...
    if(!epsHistory.length && apiKey) {
      try {
        ...
        if(cr.ok){
          ...
          raw=raw.trim().replace(/```[\s\S]*?```/g,'').trim();
          const si=raw.indexOf('['),ei=raw.lastIndexOf(']');
          if(si!==-1)epsHistory=JSON.parse(raw.slice(si,ei+1)).slice(0,4);
        }
