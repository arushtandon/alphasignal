...
function writePeriodNote(move, meta) {
  ...
  if (hideReal && hideUnreal) {
    ...
    const bits = [];
    if (!hideUnreal && Math.abs(dU) >= 0.5) bits.push('unrealised ' + moneyUsd(dU));
    if (!hideReal && Math.abs(dR) >= 0.5) bits.push('realised ' + moneyUsd(dR));
    summary = label + (bits.length ? ' the book moved ' + bits.join(' and ') : ': no material PnL change')
      + (since ? ' (' + since + ')' : '') + '. ' + vsLast;
  }
  return {
