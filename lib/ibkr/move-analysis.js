...
function contributorList(rows, limit = 4) {
  return (rows || []).slice(0, limit).map((r) => r.ticker + ' ' + moneyUsd(r.usd));
}

function periodReason(split, hideReal, hideUnreal) {
  const bits = [];
  const uHurt = !hideUnreal ? contributorList(split.unrealisedHurt) : [];
  const uHelp = !hideUnreal ? contributorList(split.unrealisedHelped) : [];
  const rHurt = !hideReal ? contributorList(split.realisedHurt) : [];
  const rHelp = !hideReal ? contributorList(split.realisedHelped) : [];
  if (uHurt.length) bits.push('Unrealised down: ' + uHurt.join(', '));
  if (uHelp.length) bits.push('Unrealised up: ' + uHelp.join(', '));
  if (rHurt.length) bits.push('Realised down: ' + rHurt.join(', '));
  if (rHelp.length) bits.push('Realised up: ' + rHelp.join(', '));
  if (!bits.length) return 'No single name moved realised or unrealised by a material amount.';
  return bits.join('. ') + '.';
}
...
function writePeriodNote(move, meta) {
  ...
  if (hideReal && hideUnreal) {
    ...
    if (!hideUnreal && startUsd != null && nowUsd != null) {
      bits.push('unrealised changed by ' + moneyUsd(nowUsd - startUsd)
        + ' (was ' + moneyUsd(startUsd) + ', now ' + moneyUsd(nowUsd) + ')');
    } else if (!hideUnreal && Math.abs(dU) >= 0.5) {
      bits.push('unrealised changed by ' + moneyUsd(dU));
    }
    if (!hideReal && Math.abs(dR) >= 0.5) bits.push('realised changed by ' + moneyUsd(dR));
    summary = label + (bits.length ? ': ' + bits.join('. ') : ': no material PnL change')
      + (since ? ' (' + since + ')' : '') + '. Main contributors — ' + reason + ' ' + vsLast;
  }
  return {
    key,
    label,
...
function buildMoveNotes(snaps, nowMs = Date.now(), opts = {}) {
  ...
  const topBits = [];
  if (day) {
    const du = (day.unrealStartUsd != null && day.unrealNowUsd != null)
      ? (Number(day.unrealNowUsd) - Number(day.unrealStartUsd)) : day.dUnrealUsd;
    topBits.push('Today: unrealised changed by ' + moneyUsd(du)
      + ', realised changed by ' + moneyUsd(day.dRealUsd) + '.');
    if (day.reason) topBits.push(day.reason);
  } else if (day && day.summary) topBits.push(day.summary);
  if (week && !week.hideReal && week.reason) topBits.push('This week — ' + week.reason);
  else if (week && week.vsLast) topBits.push('This week — ' + week.vsLast);
  if (month && !month.hideReal && month.reason) topBits.push('This month — ' + month.reason);
