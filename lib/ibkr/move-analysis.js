...
function vsLastSentence(thisMove, lastMove, lastLabel) {
  if (!lastMove || lastMove.missing) {
    return 'No ' + lastLabel + ' snapshots on file yet, so this window cannot be compared until the book has a close for that period.';
  }
  ...
  let s = 'Vs ' + lastLabel + ': realised ' + moneyUsd(tR) + ' this period vs ' + moneyUsd(lR)
    + ' (' + moneyUsd(tR - lR) + ')';
  if (lastMove.missingUnreal) {
    s += '. Unrealised marks for ' + lastLabel + ' are not on file — only realised fills were compared.';
  } else {
    s += '; unrealised ' + moneyUsd(tU) + ' vs ' + moneyUsd(lU) + ' (' + moneyUsd(tU - lU) + ').';
  }
  return s;
}
...
function writePeriodNote(move, meta) {
  ...
  if (!move || (Math.abs(dU) < 1 && Math.abs(dR) < 1)) {
    summary = label + ': no material PnL change' + (since ? ' (' + since + ')' : '') + '. ' + vsLast;
  } else {
    const bits = [];
    if (Math.abs(dU) >= 0.5) bits.push('unrealised ' + moneyUsd(dU));
    if (Math.abs(dR) >= 0.5) bits.push('realised ' + moneyUsd(dR));
    summary = label + ' the book moved ' + bits.join(' and ') + (since ? ' (' + since + ')' : '') + '. ' + vsLast;
  }
  return {
...
function buildMoveNotes(snaps, nowMs = Date.now(), opts = {}) {
  ...
  for (const spec of [starts.day, starts.week, starts.month]) {
    if (!curr) {
      periods[spec.key] = writePeriodNote(null, Object.assign({}, spec, {
        lastMove: lastPeriodMove(list, spec, fills)
      }));
      continue;
    }
    ...
    const move = (base.snap && base.snap.at !== curr.at)
      ...
      : emptyMove(base.snap && base.snap.at, curr.at);
  ...
  const topBits = [];
  if (week && (Math.abs(week.dUnrealUsd) >= 1 || Math.abs(week.dRealUsd) >= 1)) {
    topBits.push(week.summary);
  } else if (week && week.summary) {
    topBits.push(week.summary);
  }
