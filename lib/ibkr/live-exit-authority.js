...
function isLiveSignalFlipExit(evt) {
  if (process.env.IBKR_SIGNAL_FLIP_EXIT_ENABLED !== '1') return false;
  if (!evt) return false;
  if (evt.liveSignalFlip === true) return true;
  const blob = textBlob(evt);
  // `status=signal_exit` alone is historical/UI state, not permission to send a
  // market order. Require explicit live provenance for legacy payloads.
  if (blob.includes('live-signal-flip')
    && (blob.includes('signal → buy') || blob.includes('signal → sell')
      || blob.includes('signal -> buy') || blob.includes('signal -> sell'))) return true;
  return false;
}
...
function isOperationalIbkrExit(evt) {
  ...
  const needles = [
    ...
    // ib-flat-after-grace is a ledger patch, not an IB sell. Honouring it
    // cancelled Sony 6758.T SL/TP on 10 Sep after a restart ghost-flatten.
