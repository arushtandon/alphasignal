...
function classifyIbkrExitQuality(t) {
  ...
  // A server-authorized signal/time exit is executed by an IB market flatten.
  // Preserve the model trigger instead of letting the mechanical fill role hide it.
...
function fillExitPnlUsd(t, f, fx) {
  ...
  return fillCashPnlUsd(t, f, fx);
}

/** Daily cash PnL — includes futures-roll lots. Do not dump this onto "today". */
function fillDailyPnlUsd(t, f, fx) {
  if (!f || f.role === 'entry') return 0;
  return fillCashPnlUsd(t, f, fx);
}

function fillCashPnlUsd(t, f, fx) {
  const rate = Number.isFinite(Number(fx)) ? Number(fx) : 1;
  const local = Number(f && f.realizedLocal);
  if (Number.isFinite(local)) return local * rate;
  const dir = t && t.side === 'sell' ? -1 : 1;
  const qty = Number(f && f.qty) || 0;
  const px = Number(f && f.price) || 0;
  const entry = Number(t && t.avgEntry) || 0;
  const mult = Number((f && f.multiplier) || (t && t.multiplier)) || 1;
