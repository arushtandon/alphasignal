 ...
 * No last print → refuse (a restart with empty marks dumped 0883 at 24.95).
 * A stop inside minGapPct of last is treated as through (1-tick parks fill).
 */
function pickLiveTslCatchUp(input) {
  ...
  const gap = Number(input && input.minGapPct);
  const minGapPct = Number.isFinite(gap) && gap > 0 ? gap : 0.006;
  const better = (a, b) => {
    if (!(a > 0)) return b;
    if (!(b > 0)) return a;
  ...
  const unsafe = (px) => {
    if (!(lastPx > 0) || !(px > 0)) return true;
    if (isSell ? lastPx >= px : lastPx <= px) return true;
    const dist = isSell ? (px - lastPx) / lastPx : (lastPx - px) / lastPx;
    return dist < minGapPct;
  };
  const floor = better(current, floorTsl);
  let want = caught > 0 ? better(floor, caught) : floor;
  if (unsafe(want)) {
    want = unsafe(floor) ? 0 : floor;
  }
  return want;
}
