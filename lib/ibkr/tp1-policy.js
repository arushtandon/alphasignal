...
const TP1_PCT = Object.freeze({ short: 0.050, medium: 0.085, long: 0.135 });
...
/** Runner take-profit: ~1.75× the TP1 distance (same 2.8/1.6 ATR ratio). */
function synthesizeTp2Px(entry, hz, isSell) {
  const e = Number(entry);
  if (!(e > 0)) return 0;
  const pct = (TP1_PCT[hz] || TP1_PCT.short) * 1.75;
  return isSell ? e * (1 - pct) : e * (1 + pct);
}

/** Stored model TP2, else the same % further as entry→TP1, else synthesized. */
function resolveRunnerTp2Px(row) {
  const stored = Number(row && (row.tp2Px || row.modelTp2));
  if (stored > 0) return stored;
  const isSell = !!(row && row.side === 'sell');
  const entry = Number(row && (row.ibAvgFill || row.entry));
  const tp1 = Number(row && row.tp1Px);
  if (entry > 0 && tp1 > 0) {
    const move = Math.abs(entry - tp1);
    const px = isSell ? tp1 - move * 0.75 : tp1 + move * 0.75;
    if (px > 0) return px;
  }
  return synthesizeTp2Px(entry, row && row.hz, isSell);
}

function isLimitTp2Fill(input) {
  return isLimitTp1Fill(input);
}
...
/**
 * tp1Done with qtySold=0 is a false latch (9988 still held the full short
 * while TSL catch-up thought TP1 was done). Infer from IB size, or unlatch.
 */
function reconcileTp1LatchFromPosition(row, posInDir, lot) {
  const pos = Number(posInDir) || 0;
  const step = Math.max(Number(lot) || 1, 1e-8);
  const total = Number(row && row.qtyTotal) || 0;
  if (!row || !(pos > 0)) return { changed: false };
  if (!row.tp1Done || Number(row.qtySold) > 0) return { changed: false };
  if (total >= 2 * step && pos + step <= total + 1e-6 && pos < total * 0.8) {
    row.qtySold = +(total - pos).toFixed(4);
    row.qtyRunner = pos;
    return { changed: true, reason: 'infer-from-reduced-qty' };
  }
  if (total > 0 && pos >= total - step) {
    row.tp1Done = false;
    return { changed: true, reason: 'unlatch-full-lot' };
  }
  return { changed: false };
}

/**
 * Size the live STP. Never treat a planned half (`qtySold`) as a TP1 print —
 * that left 425 of 850 MNDI naked and dumped PLTR / 7733 runners. Shrink only
 * when a TP1 LMT is actually working, or TP1 is done / 100% OCA.
 */
function protectiveStopQty({ posInDir, tp1WorkingQty, tp1Done, fullTp1 }) {
  const pos = Math.max(0, Number(posInDir) || 0);
  if (fullTp1 || tp1Done) return pos;
  const parked = Math.max(0, Number(tp1WorkingQty) || 0);
  return Math.max(0, +(pos - parked).toFixed(8));
}
