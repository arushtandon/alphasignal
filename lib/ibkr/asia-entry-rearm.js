...
/** LSE: rotate venue if still unfilled — 5 venues in ~2 minutes. */
const LSE_THROUGH_SIT_MS = 25 * 1000;
...
function asiaUnfilledRearmReason(opts) {
  ...
  // User restore (6758.T 10 Sep): park OPG for the next cash open. Do not wait
  // for a live board pick — the prior ghost-flatten already closed provenance.
  if (o.userReentry && o.parentId == null) {
    if (phase === 'rth') return 'asia-rth';
    return 'asia-to-opg';
  }
  ...
  if (phase === 'rth') {
    ...
    const sitMs = Number(o.sitMs) > 0 ? Number(o.sitMs) : ASIA_THROUGH_SIT_MS;
    // LSE must print in the first 1–2 min: rotate venues even if last has not
    // crossed the parked limit (8 Sep SHEL sat on one dead LSE-direct ticket).
    if (o.forceSitRetry && age >= sitMs) return 'asia-rth-retry';
    if (asiaThroughLimitStale(o) && age >= ASIA_THROUGH_STALE_MS) return 'asia-rth-reprice';
    if (asiaThroughLimitMarketable(o) && age >= sitMs) return 'asia-rth-retry';
