...
/**
 * Calendar-roll the same IB root only when 1 lot already fits the $10k cap.
 * NYMEX BZ ×1000 is ~$87k — next month must be the ICE mini (G ×100), 1 lot.
 * Do not mint 10 minis to match full-size notional.
 */
function keepHeldSpecOnRoll(yahoo, held, entryPx) {
  const y = String(yahoo || '').toUpperCase();
  if (!COMMODITY_FUTURE_LADDER[y]) return true;
  const px = Number(entryPx) > 0 ? Number(entryPx) : (TYPICAL_PX[y] || 0);
  const pref = preferredCommoditySpec(y, px);
  if (!pref) return true;
  const heldSym = String((held && held.symbol) || '').toUpperCase();
  const heldMult = Number(held && held.multiplier) || 0;
  if (heldSym && heldSym === String(pref.symbol).toUpperCase()
      && (!heldMult || heldMult === Number(pref.multiplier))) {
    return true;
  }
  const cap = INSTRUMENT_NOTIONAL_USD * NOTIONAL_SLACK;
  const heldNotional = px * (heldMult || 0);
  return !(heldNotional > cap);
}
...
/** Keep the live month when an expired ghost reprints the same Yahoo root (BZ=F). */
function preferLiveFuturesContract(prevContract, nextContract, now) {
  if (!nextContract) return prevContract || null;
  if (!prevContract) return nextContract;
  const prevExp = futuresExpired(prevContract, now);
  const nextExp = futuresExpired(nextContract, now);
  if (nextExp && !prevExp) return prevContract;
  return nextContract;
}

/**
 * Official last-day / cash-settle prints. Yahoo `BZ=F` after expiry is the
 * *next* month — never use that as October's settlement.
 */
const FUT_CASH_SETTLE_BY_LAST_TRADE = {
  'BZ=F|20260828': 89.31
};

function officialFuturesSettlePx(yahooTicker, lastTradeDateOrContractMonth) {
  const y = String(yahooTicker || '').toUpperCase();
  if (!y) return 0;
  const d = String(lastTradeDateOrContractMonth || '').replace(/\D/g, '').slice(0, 8);
  if (d) {
    const px = Number(FUT_CASH_SETTLE_BY_LAST_TRADE[y + '|' + d]);
    return px > 0 ? px : 0;
  }
  const hits = Object.keys(FUT_CASH_SETTLE_BY_LAST_TRADE).filter(k => k.startsWith(y + '|'));
  if (hits.length === 1) {
    const px = Number(FUT_CASH_SETTLE_BY_LAST_TRADE[hits[0]]);
    return px > 0 ? px : 0;
  }
  return 0;
}

function officialFuturesSettleDate(yahooTicker) {
  const y = String(yahooTicker || '').toUpperCase();
  const hits = Object.keys(FUT_CASH_SETTLE_BY_LAST_TRADE).filter(k => k.startsWith(y + '|'));
  if (hits.length !== 1) return null;
  const d = String(hits[0].split('|')[1] || '');
...
module.exports = {
  ...
  FUT_CASH_SETTLE_BY_LAST_TRADE,
  officialFuturesSettlePx,
  officialFuturesSettleDate
};
