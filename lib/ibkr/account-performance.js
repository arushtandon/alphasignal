...
function computeAccountPerformance(input) {
  ...
  const observedEodEquity = new Map();
  ...
  for (const row of eodRows) {
    ...
    if (isPlausibleIbkrEquity(bal, liveNlv > 0 ? liveNlv : bal)) {
      ...
      observedEodEquity.set(date, +bal.toFixed(2));
      continue;
    }
  ...
  // return annualizes noise. More importantly, fill-forward weekdays are valid
  // for chart continuity but are not observed account closes: counting a run
  // of synthetic zero returns followed by one multi-day jump materially
  // distorts volatility. Use only consecutive observed EOD NLV closes.
  ...
  const sharpeCutoff = todayHasClosedEod ? today : listWeekdays(startDate || today, today).slice(-2, -1)[0];
  const observedDates = [...observedEodEquity.keys()]
    .filter((date) => !sharpeCutoff || date <= sharpeCutoff)
    .sort();
  const sessions = startDate ? listWeekdays(startDate, sharpeCutoff || today) : observedDates;
  const sessionIndex = new Map(sessions.map((date, index) => [date, index]));
  const dailyReturns = [];
  for (let i = 1; i < observedDates.length; i++) {
    const prevDate = observedDates[i - 1];
    const date = observedDates[i];
    // A gap is not a daily observation. Do not turn it into zeros plus a jump,
    // and do not pretend the whole-gap return happened in one session.
    if (sessionIndex.has(prevDate) && sessionIndex.has(date)
      && sessionIndex.get(date) - sessionIndex.get(prevDate) !== 1) continue;
    const prev = Number(observedEodEquity.get(prevDate));
    const close = Number(observedEodEquity.get(date));
    if (prev > 0 && close > 0) dailyReturns.push(close / prev - 1);
  }
  const rollingReturns = dailyReturns.slice(-252);
  const minSharpeDays = 63;
  let sharpeEstimate = null;
  if (rollingReturns.length >= 2) {
    const mean = rollingReturns.reduce((a, b) => a + b, 0) / rollingReturns.length;
    const variance = rollingReturns.reduce((s, r) => s + ((r - mean) ** 2), 0)
      / (rollingReturns.length - 1);
    const stdev = Math.sqrt(variance);
    if (stdev > 0) sharpeEstimate = +(mean / stdev * Math.sqrt(252)).toFixed(3);
  }
  // Do not publish an annualized ratio before roughly one quarter of actual
  // daily closes. The estimate can swing wildly with a handful of observations.
  const sharpeReliable = rollingReturns.length >= minSharpeDays;
  const sharpe = sharpeReliable ? sharpeEstimate : null;
  ...
  return {
    ...
    sharpeEstimate,
    sharpeReliable,
    sharpeMinDays: minSharpeDays,
    sharpeDays: rollingReturns.length,
    sharpeObservedCloses: observedDates.length,
    sharpeSince: startDate,
    sharpeIncludesToday: !!todayHasClosedEod,
    sharpeMethod: 'observed-eod-nlv-return-annualized',
    riskLevel,
    riskOff,
