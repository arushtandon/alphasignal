...
function futuresMultiplierFor(ticker, explicit) {
  ...
  // 1OZ gold is multiplier 1 — treat any positive explicit as authority.
  if (Number.isFinite(n) && n > 0) return n;
  const t = String(ticker || '').toUpperCase();
  try {
    const { ibFutMultiplier, IB_FUT_TO_YAHOO } = require('./commodity-futures');
    if (IB_FUT_TO_YAHOO[t] && ibFutMultiplier(t) > 0) return ibFutMultiplier(t);
  } catch (_) { /* optional in isolated tests */ }
