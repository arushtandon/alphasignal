...
function pickLeastBusy(slots, rrIndex, opts) {
  const ready = (Array.isArray(slots) ? slots : []).filter(s => s && s.ready && s.api);
  if (!ready.length) return null;
  const workers = ready.filter(s => !s.manager);
  const pool = (opts && opts.workersOnly && workers.length) ? workers : ready;
  let bestLoad = Infinity;
  for (const s of pool) {
    const load = Number(s.inflight) || 0;
    if (load < bestLoad) bestLoad = load;
  }
  const tied = pool.filter(s => (Number(s.inflight) || 0) === bestLoad);
...
/**
 * Tag an IB openOrder with the socket that owns it.
 * Manager reqAllOpenOrders often sends clientId 0 for worker-placed STPs.
 * Stamping the manager id there makes placeOrder/cancel hit client 27 and
 * 103-duplicate — 9988's 123.8 stop never moved.
 */
function tagOpenOrderClientId(order, slot, managerId) {
  const fromOrder = Number(order && (order.clientId != null ? order.clientId : order.clientID)) || 0;
  if (fromOrder > 0) return fromOrder;
  if (slot && slot.manager) return 0;
  const slotCid = Number(slot && slot.clientId) || 0;
  if (slotCid > 0 && slotCid !== Number(managerId)) return slotCid;
  return 0;
}

function preferWorkerOpenOrder(prev, next, managerId) {
  if (!prev) return next;
  if (!next) return prev;
  const prevCid = Number(prev.clientId) || 0;
  const cid = Number(next.clientId) || 0;
  const mgr = Number(managerId) || 0;
  if (cid > 0 && cid !== mgr && (prevCid === 0 || prevCid === mgr)) return next;
  if (cid > 0 && !(prevCid > 0)) return next;
  return prev;
}
