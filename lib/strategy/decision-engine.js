...
/** Plain sells stay off unless SELL_PICKS_ENABLED. Strong Sell is always tradable. */
function sellRecommendationAllowed(rating, sellScore, allSellsEnabled, policy = DEFAULT_POLICY) {
  if (allSellsEnabled) return true;
  const r = String(rating || '').trim().toLowerCase();
  if (r === 'strong sell') return true;
  return finiteNumber(sellScore) >= policy.strongSellScore;
}
