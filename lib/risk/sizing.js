...
/** Monday 7 Sep 2026 00:00 SGT — 3× ticket size from then, not on existing lots. */
const TICKET_SCALE_FROM_MS = Date.parse(process.env.TICKET_SCALE_FROM || '2026-09-06T16:00:00.000Z');
const TICKET_SCALE_AFTER = Math.max(1, Number(process.env.TICKET_SCALE || 3) || 3);

function liveTicketScale(nowMs = Date.now()) {
  const forced = Number(process.env.TICKET_SCALE_FORCE);
  if (Number.isFinite(forced) && forced > 0) return forced;
  const t = Number(nowMs);
  if (Number.isFinite(TICKET_SCALE_FROM_MS) && Number.isFinite(t) && t >= TICKET_SCALE_FROM_MS) {
    return TICKET_SCALE_AFTER;
  }
  return 1;
}
...
function remainingLiquidityOk(input, minNotionalUsd, nlv) {
  ...
  const floorPct = Number.isFinite(raw) && raw >= 0 ? raw : 0.25;
...
function calculateRiskSize(input, limits = DEFAULT_LIMITS) {
  ...
  const ticketScale = Math.max(1, Number(input.ticketScale ?? liveTicketScale()));
  ...
  const riskBudgetUsd = nlv * limits.riskPct * riskMultiplier * capitalScale * ticketScale;
  const riskQty = floorToLot(riskBudgetUsd / unitRiskUsd, lot);
  let maxNotionalUsd = nlv * limits.maxPositionPct * ticketScale;
  const instrumentCap = Number(input.maxNotionalUsd);
  if (instrumentCap > 0) maxNotionalUsd = Math.min(maxNotionalUsd, instrumentCap);
  ...
  // Crypto is bought in cashQty. A wide model SL (ETH 1778 vs 2511) would
  // otherwise bind on stop-risk and cut the $10k ticket to ~$4.7k.
  const quantity = input.secType === 'CRYPTO'
    ? floorToLot(Math.min(notionalQty, advQty), lot)
    : floorToLot(Math.min(riskQty, notionalQty, advQty), lot);
  if (!(quantity >= lot)) {
