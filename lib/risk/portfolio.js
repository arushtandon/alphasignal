...
const CAPITAL_UTILIZATION_POLICY_VERSION = 'available-capital-75-v1';
const MAX_CAPITAL_UTILIZATION_PCT = Math.max(
  0,
  Math.min(1, Number(process.env.MAX_CAPITAL_UTILIZATION_PCT || 0.75))
);
...
/**
 * Live entry admission requested by the portfolio owner: keep accepting entries
 * until projected capital utilisation reaches 75%. Candidate notional is charged
 * in full against AvailableFunds, which is conservative for marginable stocks.
 * Gross/net notional and geography concentration are diagnostics, not blockers.
 */
function evaluateAvailableCapitalAddition(input, maxUtilizationPct = MAX_CAPITAL_UTILIZATION_PCT) {
  const nlv = Number(input && input.nlv);
  const availableFunds = Number(input && input.availableFunds);
  const candidateCapitalUsd = Math.max(0, Number(input && input.candidateCapitalUsd) || 0);
  const cap = Math.max(0, Math.min(1, Number(maxUtilizationPct)));
  if (!(nlv > 0) || !Number.isFinite(availableFunds) || availableFunds < 0) {
    return {
      allowed: false,
      reasons: ['missing-capital-snapshot'],
      version: CAPITAL_UTILIZATION_POLICY_VERSION
    };
  }
  const currentCapitalUsedUsd = Math.max(0, nlv - availableFunds);
  const projectedCapitalUsedUsd = currentCapitalUsedUsd + candidateCapitalUsd;
  const projectedUtilizationPct = projectedCapitalUsedUsd / nlv;
  const allowed = projectedUtilizationPct <= cap;
  return {
    allowed,
    reasons: allowed ? [] : ['capitalUtilization'],
    projected: {
      nlv,
      availableFunds,
      currentCapitalUsedUsd,
      currentUtilizationPct: currentCapitalUsedUsd / nlv,
      candidateCapitalUsd,
      projectedCapitalUsedUsd,
      projectedUtilizationPct,
      remainingAvailableFunds: availableFunds - candidateCapitalUsd
    },
    maxUtilizationPct: cap,
    version: CAPITAL_UTILIZATION_POLICY_VERSION
  };
}
...
module.exports = {
  ...
  CAPITAL_UTILIZATION_POLICY_VERSION,
